...
def image_mod(input_path,output_path,width_new, height_new,size, x, y,):
    ...
    step = round(size*pi/1.7)
    for i in range(0,width,step):
        for j in range(0,height,round(step/2)):
            r = pix[i, j][0]
            g = pix[i, j][1]
            b = pix[i, j][2]
            draw.text((i,j), (num_generator(4,9)), font = font, fill = (r,g,b))
    image_new.save(output_path)
    image_new.show()

if __name__ =="__main__":
    #text("text.png",width = 2160, height = 3840, x = 1, y = 1, n = 180, size = 8)
    image_mod("input1.png", "output.png", 2160,3840, 8, 0, 0)
